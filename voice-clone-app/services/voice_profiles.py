...
def _bootstrap_profiles_dir() -> None:
    """First run of a frozen build: extract the bundled profiles/ folder
    next to the exe so the user can add/modify profiles there. Non-frozen
    runs (repo checkout) use the real profiles/ already on disk."""
    if PROFILES_DIR.exists():
        return
    bundled = BUNDLE_DIR / "profiles"
    if bundled.exists() and bundled != PROFILES_DIR:
        try:
            shutil.copytree(bundled, PROFILES_DIR)
        except OSError:
            pass


_bootstrap_profiles_dir()
