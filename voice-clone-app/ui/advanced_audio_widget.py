...
class AdvancedAudioWidget(QWidget):
    ...
    def __init__(self, config: Config, meter=None, parent=None):
        super().__init__(parent)
        self._config = config
        # Optional prepaid meter (services/metering.py). When present, the
        # pipeline is gated on a positive balance and the main window
        # stops the session if the balance runs out mid-run.
        self._meter = meter
    ...
    def set_expanded(self, expanded: bool = True):
        """Expand/collapse the panel (dedicated app starts expanded)."""
    ...
    def _start_pipeline(self):
        ...
        if self._meter is not None and not self._meter.can_afford():
            self._show_error(
                "Insufficient balance. Top up with USDT (BEP20) to continue "
                f"(${self._meter.price_usd_per_minute:.2f} per minute)."
            )
            return
    ...
    def stop_for_insufficient_balance(self):
        """Called by the main window when the prepaid meter runs out
        mid-session. Stops the pipeline and explains why."""
        if self._running:
            self._stop_pipeline()
        self._show_error(
            "Balance exhausted. Voice clone stopped — top up with USDT "
            f"(BEP20) to continue (${self._meter.price_usd_per_minute:.2f} "
            "per minute)."
        )
